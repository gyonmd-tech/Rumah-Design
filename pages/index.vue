...
onMounted(() => {
  ...
  nextTick(() => {
    if (cardsContainerRef.value) {
      setupCardsBatch(cardsContainerRef.value, '.project-card-item')
    }
  })
})
...
// Watch filter changes to refresh card triggers
watch([category, searchQuery], () => {
  nextTick(() => {
    if (cardsContainerRef.value && !isReducedMotion()) {
      setupCardsBatch(cardsContainerRef.value, '.project-card-item')
    }
  })
})
...
<template>
  <div class="relative min-h-screen bg-void">
    ...
    <section
      id="hero-section"
      ref="heroSectionRef"
      class="relative z-10 flex min-h-[96vh] sm:min-h-screen flex-col justify-between pt-36 pb-32 bg-void text-paper overflow-hidden"
    >
      <!-- Background Ambient Glow -->
      <div class="pointer-events-none absolute -top-40 left-1/2 -translate-x-1/2 w-[750px] h-[380px] bg-signal/15 rounded-full blur-[140px]" />

      <div ref="heroContentRef" class="page-shell-wide relative z-10 flex flex-col justify-center my-auto grow will-change-transform py-6">
        ...
        <div class="py-6 md:py-12">
          <h1 class="max-w-[15ch] font-display text-display-xl tracking-tight text-paper select-none">
            <span class="block overflow-hidden pt-1 pb-3 -mb-3">
        ...
        <div class="grid gap-8 border-t border-paper/15 pt-8 md:grid-cols-[1.4fr_1fr] md:items-end">
          ...
          <div ref="statsRef" class="md:text-right">
            <p class="font-mono text-xs text-paper/60 uppercase tracking-[0.14em]">
      ...
      <div
        ref="scrollCueRef"
        class="pointer-events-none absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-center transition-opacity duration-300 z-10"
        :class="isScrolled ? 'opacity-0' : 'opacity-100'"
      >
        <span class="font-mono text-[0.65rem] tracking-[0.2em] text-paper/50 uppercase">Scroll</span>
        <div class="h-8 w-[1.5px] overflow-hidden rounded-full bg-paper/20">
    ...
    <section
      id="work"
      class="relative z-20 -mt-16 sm:-mt-24 rounded-[2.5rem] md:rounded-[3.5rem] bg-paper text-ink shadow-[0_25px_80px_rgba(0,0,0,0.5)] pt-16 pb-20 mb-16 sm:mb-24 border border-ink/10"
    >
      <div class="page-shell-wide">
        ...
        <div v-if="status === 'pending' && !allProjects.length" class="grid gap-6 py-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        ...
        <!-- 4-Column Multi-Row Card Grid (8+ Curated Cards) -->
        <div
          v-else-if="filteredProjects.length"
          ref="cardsContainerRef"
          class="grid gap-6 sm:gap-7 py-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4"
        >
          <ProjectCard
        ...
        <!-- Empty Filter State -->
        <div v-else class="my-12 border border-ink/15 p-12 text-center rounded-3xl bg-white/60">
          <p class="font-display text-xl font-semibold text-ink">Belum ada project yang cocok.</p>
          <p class="mt-2 font-mono text-xs text-mute uppercase">Coba kata kunci pencarian atau kategori lain.</p>
          <button
            type="button"
